...
generate_issue_body() {
  ...
  case "$type" in
    any_type)
      prompt="TypeScript コードレビュー。

ファイル: ${file}:${line}
コード: ${code}

上記の any 型について回答してください。形式は以下の2行のみ（前置き・後書き不要）:
問題: （この any 型が引き起こす具体的な型安全性の問題を1文で）
改善案: （unknown 型または具体的な型名を使った修正例を1文で）"
      ;;
    eslint)
      prompt="ESLint エラーの分析。

ファイル: ${file}:${line}
エラー: ${code}

回答は以下の2行のみ（前置き・後書き不要）:
問題: （このエラーが発生する根本原因を1文で）
改善案: （コードの意図を保ちながら修正する具体的な方法を1文で）"
      ;;
    security)
      prompt="npm パッケージ脆弱性の評価。

パッケージ: ${file}（修正バージョン: ${line}）
脆弱性: ${code}

回答は以下の2行のみ（前置き・後書き不要）:
問題: （この脆弱性が悪用された場合の具体的なリスクを1文で）
対応: pnpm update ${file}@${line} を実行してください。（補足があれば1文以内で追記）"
      ;;
  esac
