...
prompt="以下の TypeScript 関数は行数またはネストが深く、リファクタリングを推奨します。

## 適用可能な手法（実装内容に合わせて選択）
- Extract Function: 再利用可能な処理や条件ロジックを独立した関数に抽出する
- Early Return（ガード節）: 深いネストをフラットにし、主処理を目立たせる
- Strategy / Map パターン: 多分岐の switch / if-else をオブジェクトマップで置換する
- 責務分離: I/O（DB・API）・ビジネスロジック・データ変換を別関数に切り出す
...
各関数について「関数名: 適用する手法名 + 具体的な適用箇所の説明（1〜2文）」の形式で回答してください。
前置き・後書き不要。"
